 ...
 * @author Bpatel0967
 */
public class ACBPCartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String code = (String) request.getParameter("bookCode");
        
        HttpSession session = request.getSession();
        ECart cart = (ECart)session.getAttribute("loan");
        if (cart == null) {
            cart = new ECart();
        }         
        cartitems = cart.getItems();
        
        session.setAttribute("loan", cart);
        ...
        if(action == null)
        {
            action = "";
             ...
             cart.addItem(loanItem);
             ELoan.subtractFromQOH(loanitems, code, 1);
            
            //   cartitems.add(loanItem);
        }

        String url = "/ACBPECart.jsp";
